 ...
 */
 class ListsSuite {
  ...
  @Test def `sum of a empty List should be 0`:Unit = {
  ...
  @Test def `max of a Empty List returns a NoSuchElementExcemption`:Unit = {
    try{
      max(Nil)
      Assert.fail()
    }catch{
  ...
  @Test def `max of a negative List`:Unit = {
    assertEquals(max(List(3,-1,-100000000,1000000)),1000000)
  }
